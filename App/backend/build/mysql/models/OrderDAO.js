"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.orderDAO = void 0;
const initMysql_1 = require("../initMysql");
class OrderDAO {
    getBuyerOrders(buyer) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => {
                var sqlQuery = "select * from kupovine where kupac=?";
                var queryVar = [buyer];
                initMysql_1.dbConnection.query(sqlQuery, queryVar, function (err, rows) {
                    if (err)
                        reject(err);
                    else
                        resolve(JSON.parse(JSON.stringify(rows)));
                });
            });
        });
    }
    insertOrder(order) {
        return __awaiter(this, void 0, void 0, function* () {
            var sqlQuery = "insert into kupovine(kupac, knjiga, kolicina) values (?,?,?)";
            var queryVar = [order.kupac, order.knjiga, order.kolicina];
            initMysql_1.dbConnection.query(sqlQuery, queryVar, function (err, rows) { });
        });
    }
    deleteOrder(idK) {
        return __awaiter(this, void 0, void 0, function* () {
            var sqlQuery = "delete from kupovine where idK=?";
            var queryVar = [idK];
            initMysql_1.dbConnection.query(sqlQuery, queryVar, function (err, rows) { });
        });
    }
}
exports.orderDAO = new OrderDAO();
//# sourceMappingURL=OrderDAO.js.map